...
import { type AspectRatio, isValidAspectRatio, Store } from '@foundry/domain'
import * as logger from 'firebase-functions/logger'
import { HttpsError, onCall } from 'firebase-functions/v2/https'
import { db } from '../lib/firebase'
import { FirestoreStoreRepository } from '../repositories'
import { type CreateStoreInput, CreateStoreInputSchema } from './schemas'
 ...
 * Create a new store
 */
export const createStore = onCall<CreateStoreInput>({ maxInstances: 10, timeoutSeconds: 30 }, async (request) => {
 ...
 */
export const getMyStores = onCall({ maxInstances: 20, timeoutSeconds: 10 }, async (request) => {
 ...
 */
export const getStore = onCall<{ storeId: string }>({ maxInstances: 20, timeoutSeconds: 10 }, async (request) => {
...
}>({ maxInstances: 10, timeoutSeconds: 30 }, async (request) => {
