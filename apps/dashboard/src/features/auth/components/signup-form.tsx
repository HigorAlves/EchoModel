...
export function SignupForm({ className, ...props }: React.ComponentProps<'div'>) {
	...
	const { error: authError, isSubmitting: authSubmitting, handleSignup, handleGoogleSignIn } = useAuthForm()
				...
				<CardContent className='grid p-0 md:grid-cols-2'>
					<form
						...
						<FieldGroup>
							...
							{authError && (
								<div className='bg-destructive/10 text-destructive rounded-md p-3 text-sm'>{authError}</div>
							)}
							...
							<form.Subscribe selector={(state) => [state.canSubmit, state.isSubmitting]}>
								{([canSubmit, isSubmitting]) => (
									<Field>
										<Button type='submit' disabled={!canSubmit || isSubmitting || authSubmitting}>
											{isSubmitting || authSubmitting ? t('creatingAccount') : t('createAccountButton')}
										</Button>
									</Field>
								)}
							</form.Subscribe>
							...
							<Field className='grid grid-cols-3 gap-4'>
								...
								<Button variant='outline' type='button' onClick={handleGoogleSignIn} disabled={authSubmitting}>
									<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' aria-hidden='true'>
										<path
